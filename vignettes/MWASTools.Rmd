...
<p style="font-size:110%"> First we loaded the "MWASTools" package and the "SummarizedExperiment" package: </p>
...
<p style="font-size:110%"> metabo_SE is a SummarizedExperiment object, containing 
the following information:<br> 
  ...
  - *clinical_data*: matrix containing clinical data (age, gender, type
  II diabetes and BMI) and sample type information (i.e. experimental or QC sample).
  <br><br> </p>
...
ppm <- as.numeric(colnames(metabo_matrix))
ref_spectrum <- metabo_matrix[507, ]
```
```{r tidy = TRUE, eval = FALSE}
# CV calculation 
...
QC_CV_specNMR(ref_spectrum, ppm, metabo_CV)
```
...
# Run MWAS
MWAS_BMI = MWAS_stats(metabo_SE, disease_id = "BMI", confounder_ids = c("Age","Gender", "T2D"), 
...
```{r tidy = TRUE, eval = FALSE}
STOCSY_NMR(metabo_matrix, ppm, ppm_query = 1.04)
```
